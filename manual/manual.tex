...
%% DOCUMENT
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\title{Exploration of Neural Networks \& Benchmarking of Computing Solutions
with the N2D2 Platform}
\author{Olivier Bichler, David Briand, Benjamin Bertelone}

\newif\ifidentification
%\identificationtrue
...
\begin{document}
  ...
  \begin{tikzpicture}[remember picture,overlay,anchor=west]
      \node[minimum width=\paperwidth] at ([yshift=-2.6cm,xshift=-0.1cm]current
      page.north west) {\includegraphics[width=\paperwidth]{figs/head.jpg}};
      \node[minimum width=\paperwidth] at ([yshift=2.5cm]current page.south
       west) {\textsf{\scriptsize{
        \begin{tabular*}{\linewidth}{@{\extracolsep{\fill}}ll}
          \includegraphics[height=0.5cm]{figs/list.png}
           & \includegraphics[height=0.5cm]{figs/leti.png} \\
          \color{listletiblue}Laboratoire d'Intégration des Systèmes et des
           Technologies & \color{listletiblue}Laboratoire d'Electronique et
           de Technologie de l'Information\\
           & \\
          \includegraphics[width=0.85cm]{figs/delim.jpg} & \\
          Commissariat à l'Energie Atomique et aux Energies Alternatives
          & \color{drkgreen}Direction de la Recherche Technologique\\
          Institut Carnot CEA LIST	& \color{drkgreen}{Département Architecture
          ...
          \href{mailto:thierry.collette@cea.fr}{\color{black}
          \ul{thierry.collette@cea.fr}} & \\
          \tiny{Établissement Public à caractère Industriel et Commercial
          RCS Paris B 775 685 019} & \\
        \end{tabular*}
      }}};
      \node[minimum width=\paperwidth] at ([yshift=1.4cm,xshift=9cm]current
      page.south west) {\includegraphics[width=1.11cm]{figs/isolabel.jpg}};
  \end{tikzpicture}

\vspace{5cm}

\begin{center}
\textbf{\textsf{\Large{Département\\
Architecture, Conception et Logiciels Embarqués}}}
\end{center}

\vspace{2cm}

\noindent\makebox[\textwidth][c]{\fbox{\begin{minipage}{0.8\textwidth}
\vspace{-1cm}
\textsf{\maketitle}
\end{minipage}}}
...
\fancyfoot[R]{\thepage/\pageref{LastPage}
  \begin{tikzpicture}[remember picture,overlay,anchor=west]
      \node[minimum width=\paperwidth] at ([yshift=0.8cm]current page.south
       west) {\includegraphics[width=0.95\paperwidth]{figs/foot.jpg}};
      \node[minimum width=\paperwidth] at ([yshift=0.8cm]current page.south
      west) {\textls[-50]{\scriptsize\textbf{\textcolor{white}{
        \begin{tabular}{c}
          Ce document et les informations qu'il contient, sont la propriété
          exclusive du CEA.\\
          Ils ne peuvent pas être communiqués ou divulgués sans une autorisation
           préalable du CEA LIST
        \end{tabular}
      }}}};
  \end{tikzpicture}
}
...
\begin{sffamily}
\noindent Ce document est la propriété du CEA. Il ne peut être reproduit ou
communiqué sans son autorisation.

\vspace{1cm}

\begin{tabular*}{\textwidth}{ m{4.56cm} l @{\extracolsep{\fill}}}
  IDENTIFICATION : & \\ %Rapport LIST/DACLE/2016-xx/xx \\
   & \\
   & \\
  TITRE : & \thetitle \\
   & \\
...
\noindent Ce document est la propriété du CEA. Il ne peut être reproduit ou
communiqué sans son autorisation.\\
This document is the property of the CEA. It can not be copied or disseminated
...
\begin{sffamily}
\noindent \begin{tabular}{|p{3.65cm}|*3{>{\centering\arraybackslash}p{3.65cm}|}
 @{}m{0pt}@{}}
  \cline{2-4}
   \multicolumn{1}{l|}{} & REDACTEUR \newline \emph{WRITERS} & VERIFICATEUR
   \newline \emph{CONTROLLER} & CHEF DE \newline DEPARTEMENT
   \newline \emph{HEAD of DIVISION} &\\
  \hline
  NOM & \expandafter\MakeUppercase\expandafter{\theauthor} &
  & T. COLLETTE &\\[1cm]
  \hline
  DATE & \datefr\today  & & &\\[1cm]
  \hline
  SIGNATURE &  & & &\\[1.5cm]
  \hline
\end{tabular}
...
IP\footnote{Ongoing work}. The N2D2 platform also supports the PNeuro DNN
programmable architecture\footnote{Currently at the instruction set level}.
...
; First layer: convolutionnal with 3x3 kernels
[conv1]
Input=env
Type=Conv
KernelWidth=3
...
\begin{myitemize}
    ...
    \item Cuda kernels and CuDNN code optimized for NVIDIA\textregistered{}
    GPUs.
\end{myitemize}
...
\begin{myitemize}
    \item gnuplot
    \item opencv
    \item opencv-devel (on RHEL 6, requires rhel-x86\_64-workstation-optional-6
     repository channel)
\end{myitemize}
Plus, to be able to use GPU acceleration:
\begin{myitemize}
    \item cuda
    \subitem To install cuda, add the cuda repository for RHEL 6:
\begin{lstlisting}
rpm -Uhv http://developer.download.nvidia.com/compute/cuda/repos/rhel6/x86_64/cuda-repo-rhel6-7.5-18.x86_64.rpm
yum clean expire-cache
yum install cuda
\end{lstlisting}
    Make sure the cuda library path (by default: {\tt{}/usr/local/cuda/lib64})
     is added to the {\tt{}LD\_LIBRARY\_PATH} environment variable.
    \item cudnn
    \subitem Manual installation required: register to
...
\begin{lstlisting}
./n2d2 "ini/mnist24_16c4s2_24c5s2_150_10.ini" -learn 600000 -log 10000
\end{lstlisting}
...
\begin{lstlisting}
./n2d2 "ini/mnist24_16c4s2_24c5s2_150_10.ini" -test
\end{lstlisting}
...
\begin{lstlisting}
./n2d2 "ini/mnist24_16c4s2_24c5s2_150_10.ini" -export CPP_OpenCL
\end{lstlisting}
...
\subsubsection{\texorpdfstring{%%
\lstinline[basicstyle=\ttfamily\bfseries]!PNeuro! export\protect\iponly}{PNeuro export}}
...
./test.sh -export <export_target> [-no-db-export] [-export-parameters <export_paramaters|"">] [-nbbits <number_of_bits|8>]
\end{lstlisting}
Where <export\_target> is the target system, can be \emph{C}, \emph{C++},
\emph{PNeuro}... (see below for a more detailled list), -no-db-export to disable
 the database export, <export\_paramaters> are specific parameters for each
 export, default empty, and <number\_of\_bits> is the number of bits to use
 during export, positive for integer, negative for float, default 8.


PNeuro export generates code for PNeuro and memory blocks.

The <export\_paramaters> need to be the path to an ini configuration file
which describe the global behaviour and memory accesses.
This export can only be done in 8 bits.

\paragraph{Ini configuration file}

Each sections of the ini file refers to neural network layer by its name.
Only Conv, Pool and Fc layers are supported.
The order of sections determines the order of execution on the PNeuro.
...
\begin{center}
 ...
 Option [default value] & Description\\
 \hline\hline
  \lstinline!outputFile! [pneuro.pas] & Output filename \\
  \lstinline!weightsDir! [weights] & Directory to place weights \\
  \lstinline!kernelsDir! [kernels] & Directory to place kernels \\
  \lstinline!neuroBlock! [1] & Number of NeuroBlock to use (1/2/4) \\
 \hline
\end{tabular}
\end{center}

\subparagraph{Each layers}
All layers have those parameters :
...
\begin{center}
 ...
 Option [default value] & Description\\
 \hline\hline
  \lstinline!in! [0m(0)] & Base address for input data \\
  \lstinline!inFmGap! [0] & Gap, in word, they have between each input
  feature map \\
  \lstinline!out! [0m(0)] & Base address for output data \\
  \lstinline!outFmGap! [0] & Gap, in word, to leave between each output
  feature map \\
...
\subparagraph{\emph{Convolutionnal} layers}
...
\begin{center}
 ...
 Option [default value] & Description\\
 \hline\hline
  \lstinline!weightsIn! [0m(0)] & Base address for weights \\
  \lstinline!weightsInOffset! [0] & \emph{Horizontal} offset for weights
  (0 = full left)  \\
  \lstinline!loadWeights! [onInit] & Time to load weights in memory (onInit/onStart/onTheFly) \\
  \lstinline!sign! [us] & Signs of operation, first letter for inputs,
  second weights  \\
 \hline
\end{tabular}
\end{center}
...
\begin{center}
 ...
 \hline\hline
  \lstinline!_in_after! & Address just after the last input \\
  \lstinline!_out_after! & Address just after the last output \\
  \lstinline!_weights_after! & Address just after the last weight \\
 \hline
\end{tabular}
\end{center}
...
\textit{\textbf{Limitations}}
	...
	\emph{Kernel}
		...
		\[
			1 \le Kernel_{height} \le 8 - WeightsOffset_x
		\]
	...
	\emph{Padding}
		...
		\[
			Kernel_{width} - M - 1 \le Padding_{x} \le M
		\]
...
\subparagraph{\emph{Pooling} layers}
...
\begin{center}
 ...
 Option [default value] & Description\\
 \hline\hline
  \lstinline!sign! [s] & Sign of operation  \\
  \lstinline!fullSize! [0] & Force to compute missing pixels (do not use) \\
 \hline
 \end{tabular}
\end{center}
...
\textit{\textbf{Limitations}}
	...
	\emph{Kernel \& Stride}
		...
		\[
			\begin{cases}
				1 \le Kernel_{width} \le M \\
				M \equiv 0 \pmod{Kernel_{width}} \\
				Stride_{x} = Kernel_{width}
			\end{cases}
		\]
...
\subparagraph{\emph{Fully Connected} layers}
...
\begin{center}
 ...
 Option [default value] & Description \\
 \hline\hline
  \lstinline!weightMemA! [0m(0)] & Base address for weights in first memory \\
  \lstinline!weightMemB! [0m(0)] & Base address for weights in second memory \\
  \lstinline!loadWeights! [onTheFly] & Time to load weights in memory (onInit/onStart/onTheFly)  \\
  \lstinline!sign! [ss] & Signs of operation, first letter for inputs,
   second weights  \\
  \lstinline!inputType! [auto] & Set the type of the previous layer
  (auto/2d/fc) \\
 \hline
 \end{tabular}
\end{center}
...
\begin{center}
 ...
 \hline\hline
  \lstinline!_in_after! & Address just after the last input \\
  \lstinline!_out_after! & Address just after the last output \\
  \lstinline!_temp_after! & Address just after the last temporary value \\
  \lstinline!_weightsMemA_after! & Address just after the last weight of
  first memory \\
  \lstinline!_weightsMemB_after! & Address just after the last weight of
  second memory \\
 \hline
 \end{tabular}
\end{center}
...
\begin{figure}[!htb]
  \centering
  \includegraphics[width=0.8\linewidth]{figs/nn.png}
  \caption{Example of Neural Network for PNeuro export}
  \label{fig:nnPNExport}
\end{figure}

Assiociated ini configuration file :
\begin{lstlisting}[language=ini]
neuroBlock=1
...
Memory map is generating during the export.

The file memories.data describes the memory map.

Each lines are a new subpart. They start wich the neuroblock, then the first
adress and then the size of the subpart.

The files <loadWeights>-<subpart>-<neuroblock>.data are the content of memories.
 4 LSB first then 4 MSB.
...
\paragraph{\texorpdfstring{\lstinline[basicstyle=\ttfamily\bfseries]!DistortionTransformation!\protect\iponly}
...
\begin{center}
 ...
 Option [default value] & Model(s) & Description\\
 \hline\hline
  \lstinline!Dropout! [0.5] & \lstinline!Frame_CUDA! & The probability with
