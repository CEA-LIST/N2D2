    ...
    def forward(self, inputs):
        class N2D2_computation(torch.autograd.Function):
            ...
            def forward(ctx, inputs):
                ...
                n2d2_outputs = n2d2.Tensor([], N2D2_tensor=N2D2_outputs)
                outputs = _to_torch(n2d2_outputs)
                outputs.resize_(n2d2_outputs.shape())
            ...
            def backward(ctx, grad_output):
                ...
                # n2d2_diffOutput = n2d2.Tensor([], N2D2_tensor=diffOutput)
                # outputs = _to_torch(n2d2_diffOutput)
